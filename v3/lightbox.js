(() => {
  ...
  const ccOnIcon = ccBtn?.querySelector('[data-video-icon="cc-on"]');
  const ccOffIcon = ccBtn?.querySelector('[data-video-icon="cc-off"]');
  ...
  const updateCCIcons = () => {
    const track = modalVideo.textTracks?.[0];
    const ccActive = track && track.mode === "showing";

    ccOnIcon && (ccOnIcon.style.display = ccActive ? "block" : "none");
    ccOffIcon && (ccOffIcon.style.display = ccActive ? "none" : "block");
  };
  ...
  ccBtn?.addEventListener("click", () => {
    requestAnimationFrame(updateCCIcons);
  });
