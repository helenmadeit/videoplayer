class VideoPlayer {
  constructor(wrapper, config = {}) {
    ...
    this.ccOnIcon = this.ccBtn?.querySelector('[data-video-icon="cc-on"]');
    this.ccOffIcon = this.ccBtn?.querySelector('[data-video-icon="cc-off"]');
  ...
  init() {
    ...
    this._captionsUserOverride = false;
    this._wasMuted = this.video.muted || this.video.volume === 0;

    /* --- FORCE CAPTIONS OFF ON INITIAL LOAD --- */
    /* Some browsers automatically enable <track default>.
       We explicitly disable captions at start to guarantee
       video begins with captions OFF. */
    ...
    this.bindEvents();
    requestAnimationFrame(() => this.updateCCVisibility());
  ...
  bindEvents() {
    ...
    this.ccBtn?.addEventListener("click", () => {
      ...
      /* User manually toggled captions.
         From this point forward, automatic mute-based
         behavior will no longer override their choice. */
      this._captionsUserOverride = true;
    ...
    /* --- CAPTION AUTO SYNC WITH MUTE STATE --- */
    /* Rules:
       - If user manually toggled CC → do not auto-control anymore.
       - If muted → captions ON.
       - If unmuted → captions OFF.
    */
    this.video.addEventListener("volumechange", () => {
      ...
      // Respect manual override: do not auto-sync after user interaction
      if (this._captionsUserOverride) {
        this._wasMuted = isMuted;
        return;
      }
    ...
    this.video.addEventListener("loadedmetadata", () => this.updateCCVisibility());

    const tracks = this.video.textTracks;

    if (tracks) {
      tracks.onaddtrack = () => this.updateCCVisibility();
      tracks.onremovetrack = () => this.updateCCVisibility();
    }
  ...
  updateIcons() {
    ...
    const track = this.video.textTracks?.[0];
    const ccActive = track && track.mode === "showing";
    ...
    this.ccOnIcon && (this.ccOnIcon.style.display = ccActive ? "block" : "none");
    this.ccOffIcon && (this.ccOffIcon.style.display = ccActive ? "none" : "block");
  }
  ...
  updateCCVisibility() {
    ...
    this.ccBtn.style.display = hasTracks ? "" : "none";
  }
